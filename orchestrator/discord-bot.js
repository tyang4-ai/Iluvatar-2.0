...
class DiscordBot {
    ...
    const commands = [
      ...
      new SlashCommandBuilder()
        ...
        .addAttachmentOption(opt =>
          opt.setName('pdf').setDescription('Rules PDF (optional)'))
  ...
  async handleStatus(interaction) {
    ...
    try {
      ...
      const budgetSpent = status.budget?.spent || 0;
      const budgetTotal = status.budget?.total || 100;
  ...
  async handleUploadFiles(interaction) {
    ...
    try {
      ...
      this.logger.error('Error uploading file', { error: error.message });
  ...
  async handleUploadPlan(interaction) {
    ...
    try {
      ...
      this.logger.error('Error uploading plan', { error: error.message });
  ...
  async handleContinueBuild(interaction) {
    ...
    try {
      ...
      this.logger.error('Error continuing build', { error: error.message });
  ...
  async handleModeSelection(interaction, hackathonId, mode) {
    try {
      ...
      this.logger.error('Error handling mode selection', { hackathonId, mode, error: error.message });
