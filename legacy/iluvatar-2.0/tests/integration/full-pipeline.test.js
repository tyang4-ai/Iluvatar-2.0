...
// Store subscriber and multi mocks at module level so tests can access them
let mockSubscriber;
let mockMulti;
...
describe('Full Pipeline Integration Tests', function() {
  ...
  describe('Pipeline Initialization', function() {
    it('should initialize all components for a new hackathon', async function() {
      ...
      // Initialize state (using set() method - StateManager doesn't have initialize())
      await stateManager.set('hackathon', hackathonConfig);
      ...
      // Initialize budget (use updateBudget - there's no setBudget method)
      await budgetTracker.updateBudget(hackathonConfig.budget);
      expect(mockRedis.hset.calledWith('budget:config', 'max_budget', hackathonConfig.budget)).to.be.true;
    ...
    it('should set up message bus subscriptions for all agents', async function() {
      ...
      for (const agent of agents) {
        await messageBus.subscribe(agent, () => {});
      }

      // MessageBus uses subscriber.subscribe (duplicate client)
      // Each agent subscribes to 2 channels (agent-specific + broadcast)
      expect(mockSubscriber.subscribe.callCount).to.equal(agents.length * 2);
    });
  });
  ...
  describe('Ideation Phase', function() {
    it('should handle idea approval checkpoint', async function() {
      ...
      // Track API usage for ideation (trackUsage takes agentId and usage object separately)
      await budgetTracker.trackUsage('Gandalf', {
  ...
  describe('Architecture Phase', function() {
    it('should handle Radagast architecture planning', async function() {
      ...
      // Track API usage (trackUsage takes agentId and usage object separately)
      await budgetTracker.trackUsage('Radagast', {
        model: 'claude-opus-4-20250514',
        input_tokens: 3000,
        output_tokens: 2000
      });
  ...
  describe('Development Phase', function() {
    it('should track file completions and calculate velocity', async function() {
      ...
      // Publish completion event (publish takes a message object with 'to' field)
    ...
    it('should handle agent handoffs via message bus', async function() {
      // Gimli hands off to Legolas for frontend (publish takes a message object with 'to' field)
      await messageBus.publish({
        from: 'Gimli',
        to: 'Legolas',
        type: 'handoff',
    ...
    it('should track budget consumption across multiple agents', async function() {
      // Simulate multiple agent API calls (trackUsage takes agentId and usage separately)
      const usages = [
        { agent: 'Gimli', model: 'claude-opus-4-20250514', input_tokens: 1500, output_tokens: 1000 },
        { agent: 'Legolas', model: 'claude-opus-4-20250514', input_tokens: 2000, output_tokens: 1500 },
        { agent: 'Aragorn', model: 'claude-opus-4-20250514', input_tokens: 1000, output_tokens: 800 }
      ];

      for (const { agent, ...usage } of usages) {
        await budgetTracker.trackUsage(agent, usage);
      }

      // trackUsage uses multi() for atomic operations, so check mockMulti not mockRedis
      expect(mockMulti.hincrbyfloat.callCount).to.be.greaterThan(0);
    });
  });
  ...
  describe('Budget Monitoring', function() {
    it('should warn at 80% budget consumption', async function() {
      // Mock getCurrentSpend to return 80
      mockRedis.hget.resolves('80');

      // checkBudget requires (agentId, model, estimatedInputTokens)
      const status = await budgetTracker.checkBudget('Gimli', 'claude-opus-4-20250514', 1000);

      expect(status.percentUsed).to.be.a('number');
      expect(status.currentSpend).to.equal(80);
    });
  ...
  describe('End-to-End Workflow Simulation', function() {
    it('should complete full hackathon workflow', async function() {
      // 1. Initialize (use set() - StateManager doesn't have initialize())
      await stateManager.set('hackathon', {
        id: 'e2e-hack',
      ...
      for (let i = 0; i < 5; i++) {
        ...
        await budgetTracker.trackUsage('Gimli', {
          model: 'claude-opus-4-20250514',
          input_tokens: 500,
          output_tokens: 300
        });
  ...
  describe('State Consistency', function() {
    it('should maintain consistent state across operations', async function() {
      // Initialize state (use set() - StateManager doesn't have initialize())
      await stateManager.set('hackathon', { id: 'state-test' });

      // Update state from multiple sources (use set() - StateManager doesn't have update())
      await stateManager.set('phase', 'development');
      await stateManager.set('files_completed', 10);
